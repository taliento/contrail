...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.profileForm = new FormGroup({
      ...
      'destinationPlace': new FormControl(this.session.destinationPlace, Validators.required),
      'inboundDate': new FormControl(this.session.inboundDate, Validators.required),
      'outboundDate': new FormControl(this.session.outboundDate, Validators.required)
    });
  }
  ...
  get destinationPlace() { return this.profileForm.get('destinationPlace'); }

  get inboundDate() { return this.profileForm.get('inboundDate'); }

  get outboundDate() { return this.profileForm.get('outboundDate'); }
  ...
  search = (text$: Observable<string>, originPlace: boolean) =>
    text$.pipe(
      ...
      tap(() => originPlace ? this.searching = true : this.searchingDestination = true),
      switchMap(term =>
        this.skyScanner.getPlaces(term)
        .pipe(
          tap(() => originPlace ? this.searchFailed = false : this.searchDestinationFailed = false),
          catchError((error) => {
            console.log(error);
            originPlace ? this.searchFailed = true : this.searchDestinationFailed = true;
            return of([]);
          }))

      ),
      tap(() => originPlace ? this.searching = false : this.searchingDestination = false)
    )
